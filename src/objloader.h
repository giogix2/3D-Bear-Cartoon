...
struct material{
	...
	float dif[3],amb[3],spec[3],colors[3];	//the color property (diffuse, ambient, specular)
	int illum;	//illum - we not use it
	int texture;	//the id for the texture, if there is no texture than -1
	material(const char* na,float al,float n,float ni2,float* d,float* a,float* s,float* col,int i,int t);
};
...
class objloader{
	...
	void loadShapeToVector(const char* filename);
	void draw();
	void setSibling(objloader *obj);
	objloader *getSibling();
	void setChild(objloader *obj);
	objloader *getChild();
	int getSiblingNum();
	int getChildrenNum();
...
// Algorithm which permits to drw each peace just once
inline void traverse(objloader *root) {
		if(root == NULL) return;
		glPushMatrix();
		root->transform();
		root->draw();
		if(root->getChildrenNum() != 0) {
			traverse(root->getChild());
		}
		glPopMatrix();
		if(root->getSiblingNum() != 0) {
			traverse(root->getSibling());
		}
	}
