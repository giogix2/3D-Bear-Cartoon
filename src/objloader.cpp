...
#include "objloader.h"
	...
	//nothing to explain here
	material::material(const char* na,float al,float n,float ni2,float* d,float* a,float* s,float* col,int i,int t)
	{
		name=na;
		alpha=al;
		ni=ni2;
...
void objloader::loadShapeToVector(const char* filename) {
	...
	for(int i=0;i<coord.size();i++)	//and then go through all line and decide what kind of line it is
	{
		...
		if((*coord[i])[0]=='v' && (*coord[i])[1]==' ')	//if a vertex
		{
		...
		else if((*coord[i])[0]=='m' && (*coord[i])[1]=='t' && (*coord[i])[2]=='l' && (*coord[i])[3]=='l')	//material library, a file, which contain																																																		//all of the materials
	{
		...
		float amb[3],dif[3],spec[3],colors[3],alpha,ns,ni;	//colors, shininess, and something else
		int illum;
		...
		for(int i=0;i<tmp.size();i++) //go through all lines of the mtllib file
		{
			...
			if(tmp[i][0]=='n' && tmp[i][1]=='e' && tmp[i][2]=='w')	//new material
			{
				if(ismat)	//if we have a material
				{
					...
					{
						materials.push_back(new material(name,alpha,ns,ni,dif,amb,spec,colors,illum,texture));	//push back
						strcpy(filename,"\0");
					}else{
							materials.push_back(new material(name,alpha,ns,ni,dif,amb,spec,colors,illum,-1));		//push back, but use -1 to texture		
					}
				}
			...
			else if(tmp[i][0]=='C' && tmp[i][1]=='l')	//and the colors
			{
				sscanf(tmp[i].c_str(),"Cl %f %f %f",&colors[0],&colors[1],&colors[2]);
				ismat=true;
			}
		}
		if(ismat)	//there is no newmat after the last newmat, so we have to put the last material 'manually'
		{
			...
			{
				materials.push_back(new material(name,alpha,ns,ni,dif,amb,spec,colors,illum,texture));
			}else{
					materials.push_back(new material(name,alpha,ns,ni,dif,amb,spec,colors,illum,-1));				
			}
		}
	}
	...
	for(int i=0;i<faces.size();i++)	//go throught all faces
	{
		if(faces[i]->four)	//if quad
		{
			...
			glEnd();
				...
				z3 = vertex[faces[i]->faces[2]-1]->z;

				Triangle triangle_temp = Triangle(x1, y1, z1, x2, y2, z2, x3, y3, z3);
				

				if(isnormals)	//if there are normals
					triangle_temp.setNormal(normals[faces[i]->facenum-1]->x, normals[faces[i]->facenum-1]->y, normals[faces[i]->facenum-1]->z);
				triangle_temp.setDiffuse(materials[faces[i]->mat]->dif[0], materials[faces[i]->mat]->dif[1], materials[faces[i]->mat]->dif[2]);
				triangle_temp.setSpecular(materials[faces[i]->mat]->spec[0], materials[faces[i]->mat]->spec[1], materials[faces[i]->mat]->spec[2]);
				triangle_temp.setAmbient(materials[faces[i]->mat]->amb[0], materials[faces[i]->mat]->amb[1], materials[faces[i]->mat]->amb[2]);
				triangle_temp.setShininess(materials[faces[i]->mat]->ns);
				triangle_temp.setColor(materials[faces[i]->mat]->colors[0], materials[faces[i]->mat]->colors[1], materials[faces[i]->mat]->colors[2]);
				//glNormal3f(normals[faces[i]->facenum-1]->x,normals[faces[i]->facenum-1]->y,normals[faces[i]->facenum-1]->z);
				triangles.push_back(triangle_temp);
		}
...
void objloader::setSibling(objloader *obj) {
	sibling = obj;
	siblings_num++;
}

objloader *objloader::getSibling() {
	return sibling;
}

void objloader::setChild(objloader *obj) {
	child = obj;
	children_num++;
}

objloader *objloader::getChild() {
	return child;
}

int objloader::getSiblingNum() {
	return siblings_num;
}

int objloader::getChildrenNum() {
	return children_num;
}

void objloader::setRotation(float x, float y, float z) {
	rotation = glm::vec3(x,y,z);
}

void objloader::setTranslation(float x, float y, float z) {
	translation = glm::vec3(x,y,z);
}

void objloader::transform() {
	glTranslatef(translation.x, translation.y, translation.z);
	glRotatef(rotation.x, rotation.y, rotation.z, 1.0f);
}